...
####################################################################
# Start script used to launch the development image interactively  #
...
help()
{
    ...
    launch the development image interactively with common \
    development arguments"
    echo 
    echo "Syntax: start [-h|p|e|s path/to/source|t path/to/target|v /path/to/device]"
    echo "Options:"
    echo "h     Print this help message"
    echo "p     Pull the latest version of the development image prior to use"
...
pull_latest_version()
{
    docker pull ghcr.io/unl-nimbus-lab/arl-swarm/development:main
}


run()
{
    docker run -it "$@" ghcr.io/unl-nimbus-lab/arl-swarm/development:main
}
...
while getopts ":hpes:t:v:" opt; do
    case "${opt}" in
        h)
            ...
            ;;
        p)
            echo "[INFO] Pulling latest development image version"
            pull_latest_version
            ;;
        e)
            HOST_NET="--net=host"
